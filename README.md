...
Crea un archivo `.env` local a partir de `.env.example`. **No compartas ni subas el archivo `.env` real**, ya que puede contener credenciales de MongoDB.
...
### 3. Ejecución
    ...
    # Modo desarrollo (con recarga automática mediante tsx/nodemon)
    npm run dev
...
| `PUT`  | `/api/estudiantes/:id` | Actualizar registro |
| `DELETE`| `/api/estudiantes/:id`| Eliminar estudiante |

*(Nota: Los endpoints CRUD para `/api/estudiantes/programas` y `/api/estudiantes/documentos` siguen exactamente la misma convención REST. Consultar Swagger para más detalles).*
...
- **Filtros:** `?ciudad=Tunja&programa={id}&search=laura`
...
3. **Población (Populate):** Al consultar un estudiante por ID, Mongoose automáticamente inyecta el objeto completo del programa académico asociado. El Gateway no necesita hacer una segunda petición a `/api/estudiantes/programas`.
4. **Relación de Documentos:** Para obtener los documentos de identidad asociados a un estudiante, se puede realizar un `GET` a `/api/estudiantes/documentos?estudianteId={id}`.
